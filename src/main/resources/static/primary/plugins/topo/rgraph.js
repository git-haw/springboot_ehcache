...
!function (global) {
    ...
    (!Raphael.fn.rgraphnode ) && (Raphael.fn.rgraphnode=function(node){
        ...
        node.point[1] = node.point[1] ||0;console.log(node.point[1]);
        
        switch (node.type){
          case 'image' :
            node._shape = paper.image(node.src,node.posx || node.point[0]-size.w/2,node.poxy || node.point[1]-size.h/2, node.width || 50, node.height || 50)
            .attr({stroke: "#ccc",fill:"#ccc","stroke-width":1,"cursor":"pointer"})
            .attr(node.rectStyle||{});
            node._text = paper.text(node.posx+30 || node.point[0],node.poxy+node.height || node.point[1]+node.height,text).attr({"cursor":"pointer","font-size":12}).attr(node.textStyle||{});
            break;

          case 'circle' :
            node._shape = paper.circle(node.posx || node.point[0]-size.w/2,node.poxy || node.point[1]-size.h/2, node.width || 23, node.height || 23)
            .attr({stroke: "#ff4640",fill:"#fff","stroke-width":1,"cursor":"pointer"})
            .attr(node.rectStyle||{});
            node._text = paper.text(node.posx || node.point[0],node.poxy || node.point[1],text).attr({"cursor":"pointer","font-size":12}).attr(node.textStyle||{});
            break;

          default :
            node._shape = paper.rect(node.posx || node.point[0]-size.w/2,node.poxy || node.point[1]-size.h/2, size.w, size.h,3)
            .attr({stroke: "#ccc",fill:"#ccc","stroke-width":1,"cursor":"pointer"})
            .attr(node.rectStyle||{});
